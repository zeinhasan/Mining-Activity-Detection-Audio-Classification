...
input_folder_yes = "Dataset/Yes"
output_folder_yes = "Dataset/Yes/Augmented"
# Augmentation parameters
replication_factor = 10
desired_duration = 15
# Create the output folder if it doesn't exist
if not os.path.exists(output_folder_yes):
    os.makedirs(output_folder_yes)
    # Loop over each audio file in the input folder
for filename in os.listdir(input_folder_yes):
    if filename.endswith('.mp3'):
        # Load audio file using pydub
        audio_file = os.path.join(input_folder_yes, filename)
        ...
        for augmentation, augmented_audio in augmented_files:
            for i in range(replication_factor):
                ...
                output_file = os.path.join(output_folder_yes, output_filename)
